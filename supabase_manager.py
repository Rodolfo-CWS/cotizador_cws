...
class SupabaseManager:
        ...
        try:
            ...
            if datos.get('tipo') == 'simple' and datos.get('plan'):
                ...
                if max_pdfs is not None:
                    usados = self.contar_pdfs_mes(company_id)
                    if usados >= max_pdfs:
                        return {
                            "success": False,
    ...
    def generar_numero_cotizacion(self, cliente, vendedor, proyecto, revision=1, company_code=None):
        ...
        try:
            ...
            iniciales_vendedor = ""
            if vendedor:
                vendedor_normalizado = unicodedata.normalize('NFKD', vendedor).encode('ASCII', 'ignore').decode('ASCII').upper()
                palabras = vendedor_normalizado.strip().split()
    ...
    def registrar_pdf_simple(self, company_id: str) -> bool:
        """Registra una cotización simple creada (para la cuota mensual de PDFs)."""
        if not company_id:
            return False
        ...
        try:
            if self.pg_connection and not self.pg_connection.closed:
                ...
                cursor = self.pg_connection.cursor()
                cursor.execute(
                    """INSERT INTO public.simple_pdf_usage (company_id)
                       VALUES (%s)""",
                    (company_id,)
                )
                self.pg_connection.commit()
            ...
            print(f"[PDF_SIMPLE] Error registrar uso (PG): {e}")
            try:
                self.pg_connection.rollback()
            except:
                pass
        ...
        try:
            ...
            if url and key:
                client = create_client(url, key)
                client.table('simple_pdf_usage').insert({'company_id': company_id}).execute()
                return True
        except Exception as e:
            print(f"[PDF_SIMPLE] Error registrar uso (SDK): {e}")

        return False

    def contar_pdfs_mes(self, company_id: str) -> int:
        """Cuenta las cotizaciones simples del mes calendario actual."""
        if not company_id:
            return 0
        ...
        try:
            if self.pg_connection and not self.pg_connection.closed:
                ...
                cursor = self.pg_connection.cursor()
                cursor.execute(
                    """SELECT COUNT(*) FROM public.simple_pdf_usage
                       WHERE company_id = %s
                         AND created_at >= date_trunc('month', NOW())""",
                    (company_id,)
                )
            ...
            print(f"[PDF_SIMPLE] Error contando uso (PG): {e}")
            try:
                self.pg_connection.rollback()
            except:
                pass
        ...
        try:
            ...
            if url and key:
                ...
                resp = client.table('simple_pdf_usage') \
                    .select('id') \
                    .eq('company_id', company_id) \
                    .gte('created_at', month_start) \
                    .execute()
            ...
            print(f"[PDF_SIMPLE] Error contando uso (SDK): {e}")
